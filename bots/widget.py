...
def translate_to_lang(text: str, lang: str) -> str:
    ...
    try:
        ...
        return (res.choices[0].message.content or "").strip() or text
    except Exception as exc:
        print(f"[LANG TRANSLATE] Failed: {exc}")
        return text


def build_html_body(description: str, user_email: str) -> str:
...
<style>
  body {{ font-family: 'Segoe UI', Arial, sans-serif; background:#f4f6f8; margin:0; padding:30px 15px; color:#333; }}
  .container {{ max-width:600px; margin:auto; background:#fff; border-radius:10px; box-shadow:0 4px 15px rgba(0,0,0,.05); padding:30px; }}
  .title {{ font-size:22px; font-weight:600; color:#000; margin-bottom:25px; text-align:center; }}
  .card {{ display:flex; border-radius:8px; overflow:hidden; border:1px solid #e5e7eb; }}
  .card-accent {{ width:6px; background:#2563eb; }}
  .card-content {{ padding:20px; flex:1; }}
  .row {{ margin-bottom:18px; }}
  .label {{ font-weight:600; color:#111827; margin-bottom:6px; }}
  .value {{ background:#f9fafb; padding:10px 12px; border-radius:6px; border:1px solid #e5e7eb; font-size:14px; }}
  .footer {{ margin-top:30px; font-size:12px; color:#6b7280; text-align:center; }}
</style></head>
<body>
  <div class="container">
    <div class="title">Digital Coaching Support Request</div>
    <div class="card">
      <div class="card-accent"></div>
      <div class="card-content">
        <div class="row"><div class="label">User email:</div><div class="value"><strong>{user_email or 'Not provided'}</strong></div></div>
        <div class="row"><div class="label">Request received on:</div><div class="value">{current_date}</div></div>
        <div class="row"><div class="label">Problem description:</div><div class="value">{description_escaped}</div></div>
      </div>
    </div>
    <div class="footer">© 2026 Digital Coaching - Technical Support</div>
  </div>
</body></html>"""


def send_support_email(description: str, user_email: str) -> bool:
    msg = MIMEMultipart("alternative")
    msg["Subject"] = "Digital Coaching Support Request"
    ...
    text = "\n".join([
        "=" * 50, "DIGITAL COACHING SUPPORT REQUEST", "=" * 50,
        f"User email: {user_email or 'unknown'}",
        f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        "-" * 50, "PROBLEM DESCRIPTION", "-" * 50,
        (description or "").strip(), "=" * 50,
    ])
    msg.attach(MIMEText(text, "plain", "utf-8"))
    msg.attach(MIMEText(build_html_body(description, user_email), "html", "utf-8"))
    ...
    try:
        ...
        print(f"[SUPPORT EMAIL] Failed: {exc}")
        return False


def polish_description(details: str) -> str:
    try:
        res = client.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": (
                    "You are an editor. Fix grammar, spelling, and punctuation ONLY. "
                    "Do NOT add or remove information. Do NOT paraphrase. Do NOT ask questions. "
        ...
        return (res.choices[0].message.content or "").strip() or details
    except Exception as exc:
...
def run(message: str, session: dict) -> str:
    ...
    session contient maintenant :
      - stage      : "idle" | "support_waiting_details"  (envoyé par le client)
      - user_email : email de l'utilisateur connecté
    session est MUTÉ ici → main.py le relit après pour retourner le nouveau stage au client.
    """
    stage = session.get("stage", "idle")
    user_email = session.get("user_email") or ""
    lang = detect_language(message)

    # STAGE 1 : l'utilisateur vient de décrire son problème → envoyer l'email
    if stage == "support_waiting_details":
        description = polish_description(message.strip())
        ok = send_support_email(description=description, user_email=user_email)
        session["stage"] = "idle"          # ← reset, sera renvoyé au client
        if ok:
    ...
    # STAGE 0 : détecter une demande de support
    if stage == "idle" and _is_support_request_llm(message):
        session["stage"] = "support_waiting_details"   # ← sera renvoyé au client
        return translate_to_lang(
